...
export function GameScheduleCard({ event, onOpen, isPast = false }: Props) {
  const assistantLabel = event.assistantNames.length > 0 ? event.assistantNames.join(', ') : '—';
  const accentColor = isPast ? PAST_ACCENT : BRAND.amber;
  const actionLabel = isPast ? 'View Game' : 'Build Roster';
  ...
  return (
    ...
    >
      <Box sx={{ width: '100%' }}>
        ...
        >
          <ColoredTag label="Game" color={accentColor} />
